...
> PostgreSQL - это бесплатная open-source реляционная СУБД использующая SQL
...
> Кластер - это коллекция баз данных, которые управляются единственным экземпляром запущенного сервера. Нет, это не нейрослоп. Это попытка перевести определение с [английского](https://docs.postgresql.tw/server-administration/runtime/creating-cluster).
...
> DBeaver - это бесплатный open-source графический инструмент для работы с базами данных.
